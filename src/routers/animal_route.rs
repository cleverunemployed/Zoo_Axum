...
use crate::{
    handlers::prelude::*, repositories::animal_repository::AnimalRepository,
    services::animal_service::AnimalService, state::AnimalState,
};
...
impl RouterAnimal {
    pub fn new(pool: Arc<PgPool>) -> Router {
        let state = AnimalState {
            service: AnimalService::new(AnimalRepository::new(pool)),
        };

        let app = Router::new()
            .route("/animals", post(get_all_animals))
