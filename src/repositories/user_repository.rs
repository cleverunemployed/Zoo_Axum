...
use crate::{models::users::{ParamsForUsers, User}, schemas::{CreateUserRequest, UpdateUserRequest, UserResponse}};
...
impl UserRepository {
    ...
    pub async fn create_user(self, data: CreateUserRequest) -> Result<UserResponse, Error> {
        let response = sqlx::query_as!(
            UserResponse,
            "INSERT INTO users (email, password, role, is_deleted) VALUES ($1, $2, $3, $4) RETURNING id, email, role",
    ...
    pub async fn delete_user(self, id: i32) -> Result<i32, Error> {
        let result = sqlx::query!("UPDATE users SET is_deleted = true WHERE id = $1", id)
            .execute(&*self.pool)
            .await?;
    ...
    pub async fn update_user(self, data: UpdateUserRequest) -> Result<UserResponse, Error> {
        ...
        if !needs_comma {
            return Err(Error::ColumnNotFound("Для обновления пользователя нужны данные!".to_string()));
        }
